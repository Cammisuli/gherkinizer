...
class Gherkinizer {
        ...
        try {
            ...
            do {
                ...
                for (const filePath of filePaths) {
                    ...
                    const fileChanged = await this._outputFile(filePath, steps);
                    repeat = repeat || fileChanged;
                }
                helpers_1.log('Finished generating files, another pass required: ' + repeat);
            } while (repeat);
            if (this.WATCH_MODE) {
    ...
    async _updateFileHash(filePath, content = '') {
        return new Promise((resolve) => {
            ...
            const oldHash = this._fileHashes.get(key);
