...
class Gherkinizer {
    ...
    constructor(GLOB_PATH, STEPS, PATH_OUT_DIR, WATCH_MODE = false, VERBOSE = false) {
        ...
        this._stepFileContentHashes = new Map();
        // nothing needed
    }
    /**
    ...
    _checkForStepFileChanges() {
        let changed = false;
        glob.sync(this.STEPS).forEach((filePath) => {
            const fileContents = fs.readFileSync(filePath).toString();
            const hash = md5_1.Md5.hashAsciiStr(fileContents).toString();
            if (!this._stepFileContentHashes.has(filePath)) {
                helpers_1.log('Contents hash added for: ' + filePath);
                changed = true;
            }
            else if (this._stepFileContentHashes.get(filePath) !== hash) {
                helpers_1.log('Contents hash changed for: ' + filePath);
                changed = true;
            }
