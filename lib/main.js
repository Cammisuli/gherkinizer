...
class Gherkinizer {
    ...
    async _start(steps, reWatchSteps = false) {
        try {
            ...
            do {
                ...
                helpers_1.log('Finishing building steps, another pass required: ' + this._changedStepFiles);
    ...
    _watchFiles(steps) {
        ...
        featureWatcher.on('change', (filePath) => {
            helpers_1.log('feature file changed: ' + filePath);
            this._outputFile(filePath, steps);
        });
        featureWatcher.on('add', (filePath) => {
            helpers_1.log('feature file added: ' + filePath);
            this._outputFile(filePath, steps);
        });
        const stepWatcher = new watcher_1.default([this.STEPS]);
        stepWatcher.on('change', (filePath) => {
            helpers_1.log('Step file changed: ' + filePath);
            featureWatcher.close();
            stepWatcher.close();
            this._start(steps);
        });
        stepWatcher.on('add', (filePath) => {
            helpers_1.log('Step file added: ' + filePath);
            featureWatcher.close();
            stepWatcher.close();
            this._start(steps);
        });
    ...
    async _addNewStepFileHashes() {
        return new Promise(async (resolve) => {
            for (const filePath of glob.sync(this.STEPS)) {
                const key = this._hashKey(filePath);
    ...
    async _updateStepFileHash(filePath, content = '') {
        return new Promise((resolve) => {
            ...
            const key = this._hashKey(filePath);
            const oldHash = this._stepFileHashes.get(key);
